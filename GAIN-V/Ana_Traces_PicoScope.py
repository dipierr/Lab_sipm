...
'''
Goal: to extract information from SiPM traces digitized by the PicoScope.
Steps: 
...
PARSER.add_argument('-last', '--last_event',type=int, required=False, default=1, help='Last event number to analyze')
PARSER.add_argument('-miny', '--miny', type=float, required=False, default=-0.002, help='Amplitude signal scale (y), minimum')
PARSER.add_argument('-maxy', '--maxy', type=float, required=False, default= 10, help='Amplitude signal scale (y), maximum')
...
def main(**kwargs):	
	...
	plt.grid(True)
	plt.hist(peaks_all, bins = 80, range = (0, 10), facecolor='g', edgecolor='g',histtype='step', stacked=True, fill=False)
	plt.yscale('log')
	plt.xlabel('V')	
	plt.show()
